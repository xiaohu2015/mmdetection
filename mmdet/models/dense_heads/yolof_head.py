...
from mmdet.core import images_to_levels, multi_apply, reduce_mean
...
class UniformMatcher(nn.Module):
    ...
    def forward(self, pred_boxes, anchors, targets):
        ...
        indices = [
            tuple(
                torch.topk(c[i], k=self.match_times, dim=0,
...
class YOLOFBox2BoxTransform(object):
    ...
    parameterized by 4 deltas: (dx, dy, dw, dh). The transformation scales the
    box's width and height by exp(dw), exp(dh) and shifts a box's center.
    ...
    def get_deltas(self, src_boxes, target_boxes):
        """Get box regression transformation deltas (dx, dy, dw, dh) that can
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        if self.add_ctr_clamp:
            dx_width = torch.clamp(
                dx_width, max=self.ctr_clamp, min=-self.ctr_clamp)
            dy_height = torch.clamp(
                dy_height, max=self.ctr_clamp, min=-self.ctr_clamp)
...
def permute_to_N_HWA_K(tensor, K: int):
    """Transpose/reshape a tensor from (N, (Ai x K), H, W) to (N, (HxWxAi),
    K)"""
...
class Boxes:
    """This structure stores a list of boxes as a Nx4 torch.Tensor. It supports
    some common methods about boxes (`area`, `clip`, `nonempty`, etc), and also
    behaves like a Tensor (support indexing, `to(device)`, `.device`, and
    iteration over all boxes)

    Attributes:
        tensor (torch.Tensor): float matrix of Nx4. Each row is
         (x1, y1, x2, y2).
    """
    ...
    def __init__(self, tensor: torch.Tensor):
        ...
        if tensor.numel() == 0:
            # Use reshape, so we don't end up creating a new t
            # ensor that does not depend on
            # the inputs (and consequently confuses jit)
            tensor = tensor.reshape((-1, 4)).to(
    ...
    def clip(self, box_size: Tuple[int, int]) -> None:
        """Clip (in place) the boxes by limiting x coordinates to the range [0,
        width] and y coordinates to the range [0, height].

        Args:
            box_size (height, width): The clipping box's size.
        """
        assert torch.isfinite(
            self.tensor).all(), 'Box tensor contains infinite or NaN!'
        h, w = box_size
    ...
    def nonempty(self, threshold: float = 0.0) -> torch.Tensor:
        """Find boxes that are non-empty. A box is considered empty, if either
        of its side is no larger than threshold.
    ...
    def __getitem__(self, item) -> 'Boxes':
        ...
        1. `new_boxes = boxes[3]`: return a `Boxes` which
        contains only one box.
        2. `new_boxes = boxes[2:10]`: return a slice of boxes.
        3. `new_boxes = boxes[vector]`, where vector is a
        torch.BoolTensor
           with `length = len(boxes)`. Nonzero elements in the vector
           will be selected.
    ...
    def inside_box(self,
        ...
        Args:
            ...
            boundary_threshold (int): Boxes that extend beyond the
            reference box
                boundary by more than boundary_threshold are
                considered "outside".

        Returns:
            a binary vector, indicating whether each box
            is inside the reference box.
        """
        height, width = box_size
        inds_inside = ((self.tensor[..., 0] >= -boundary_threshold)
    ...
    def cat(cls, boxes_list):
        ...
        # use torch.cat (v.s. layers.cat) so the returned boxes never
    ...
    # type "Iterator[torch.Tensor]", yield, and iter() not supported
    # by torchscript
