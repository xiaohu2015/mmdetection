...
class TTFNetHead(AnchorHead):
    ...
    def __init__(self,
        ...
        for i in range(2, len(planes)):
            self.deconv_layers.append(
                self.build_upsample(
                    planes[i - 1], planes[i], norm_cfg=norm_cfg))
    ...
    def get_bboxes(self, pred_heatmap, pred_wh, img_metas, rescale=False):
        batch, cat, height, width = pred_heatmap.size()
        pred_heatmap = pred_heatmap.sigmoid_()
        wh = pred_wh

        # perform nms on heatmaps
        heat = self._local_maximum(
            pred_heatmap)  # used maxpool to filter the max score

        topk = getattr(self.test_cfg, 'max_per_img', 100)
        # (batch, topk)
        ...
        if not self.wh_agnostic:
            ...
            wh = torch.gather(
                wh, 2, clses[..., None,
                             None].expand(clses.size(0), clses.size(1), 1,
                                          4).long())
        ...
        result_list = []
        score_thr = getattr(self.test_cfg, 'score_thr', 0.01)
    ...
    def _topk(self, scores, topk=20):
        ...
        topk_scores, topk_inds = torch.topk(scores.view(batch, -1), topk)
    ...
    def target_single_image(self, gt_boxes, gt_labels, feat_shape):
        ...
        for k in range(boxes_ind.shape[0]):
            ...
            if self.wh_gaussian:
                ...
                box_target_inds = torch.zeros_like(
                    fake_heatmap, dtype=torch.uint8)
    ...
    def target_generator(self, gt_boxes, gt_labels, img_metas):
        ...
        with torch.no_grad():
            ...
            heatmap, box_target = [
                torch.stack(t, dim=0).detach() for t in [heatmap, box_target]
            ]
...
class ShortcutConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        layers = []
        for i, (kernel_size,
                padding) in enumerate(zip(kernel_sizes, paddings)):
